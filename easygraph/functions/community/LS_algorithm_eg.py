...
def plot_combination(x, y, text, x1, y1, text1, center_id, subplot_location, xlim_start_end, ylim_start_end,
                     font_location, filepath='./', dataname='LS_default', save=False):
    '''
    input:
        x：节点的度值(数据类型：list)k
...
def hierarchical_degree_communities(G_LS, center_num=None, auto_choose_centers=False, maximum_tree=True, seed=None,
                                    self_loop=False):
    '''
...
if __name__ == '__main__':
    ...
    # If you want to use your own dataset, use networkx to read and set label = "id" e.g. nx.read_gml("your dataset",label="id")
    # G=nx.read_gml("net_SBM_compact_nb_groups_100_block_size_5_p_in_0.8_k_out_8_i_0.gml",label="id")
